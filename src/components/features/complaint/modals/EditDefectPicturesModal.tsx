...
export const EditDefectPicturesModal: React.FC<EditDefectPicturesModalProps> = ({
  ...
  const handleFilesSelect = async (files: FileList | null) => {
    ...
    for (const file of fileArray) {
      ...
      try {
        ...
        setNewPictures((prev) =>
          prev.map((p) =>
            p.id === id ? { ...p, isUploading: false, error: err.message || 'Tải ảnh thất bại' } : p
          )
        );
      }
    }
  ...
  const handleSave = async () => {
    ...
    if (isAnyUploading) {
      setErrorMessage('Vui lòng đợi các hình ảnh hoàn tất tải lên MinIO trước khi lưu.');
      return;
    }
    ...
    try {
      // 1. Kết hợp danh sách URL hiện tại + bất kỳ URL thủ công nào vừa nhập
      const additionalUrls = parsePictureUrls(customUrlsText);
      const allUrls = [...existingUrls, ...additionalUrls];

      // 2. Lấy danh sách tmpKey của các ảnh mới tải lên
      ...
      setErrorMessage(err.message || 'Không thể cập nhật ảnh khiếu nại. Vui lòng thử lại.');
    } finally {
      setIsSaving(false);
    }
  };
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-text-primary">
                Quản lý hình ảnh lỗi khiếu nại (MinIO Storage)
              </h3>
              <p className="text-[11px] text-text-muted">
                Hồ sơ: #{complaint.id} • Mã theo dõi: <span className="font-mono font-semibold">{complaint.trackingNo}</span>
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto space-y-5 text-xs">
          ...
          <div className="space-y-2">
            <label className="block font-semibold text-text-secondary">
              Tải thêm ảnh lỗi từ máy tính / điện thoại:
            </label>
            <div
            ...
            >
              ...
              <div className="flex items-center justify-center gap-2 text-primary font-semibold">
                ...
                <span>Nhấp để chọn hoặc kéo thả nhiều ảnh lỗi vào đây</span>
              </div>
              <p className="text-[11px] text-text-muted">
                Ảnh sẽ được tải trực tiếp lên MinIO (`complaints/{complaint.year}/{complaint.trackingNo}/defects/`)
              </p>
            </div>
          </div>
          ...
          {newPictures.length > 0 && (
            <div className="space-y-2">
              <span className="text-[11px] font-semibold text-text-secondary block">
                Ảnh mới tải lên ({newPictures.length}):
              </span>
          ...
          <div className="space-y-2">
            <span className="text-[11px] font-semibold text-text-secondary block">
              Ảnh hiện có trong hồ sơ ({existingUrls.length}):
            </span>
            {existingUrls.length > 0 ? (
              ...
              <div className="p-4 rounded-xl bg-surface-canvas/60 border border-border-subtle text-center text-text-muted text-[11px]">
                Chưa có ảnh lỗi nào trong hồ sơ này.
              </div>
            )}
          </div>
          ...
          <div className="space-y-1.5 pt-3 border-t border-border-subtle">
            <label className="block font-semibold text-text-secondary">
              Hoặc dán thêm URL ngoài (tùy chọn):
            </label>
            <textarea
              rows={2}
              value={customUrlsText}
              onChange={(e) => setCustomUrlsText(e.target.value)}
              placeholder="Dán URL ảnh ngoài nếu có (phân cách bằng dấu phẩy hoặc xuống dòng)..."
